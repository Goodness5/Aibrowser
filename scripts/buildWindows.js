...
async function afterPackageBuilt (packagePath) {
  ...
  /* create zip files */
  var output = fs.createWriteStream('dist/app/' + 'Aibrowser-v' + version + '-windows' + (packagePath.includes('ia32') ? '-ia32' : '') + '.zip')
  ...
  if (!packagePath.includes('ia32')) {
    ...
    const options = {
      src: packagePath,
      dest: 'dist/app/Aibrowser-installer-x64',
    ...
    await installer(options)
      .then(function () {
        fs.renameSync('./dist/app/Aibrowser-installer-x64/min-' + version + '-setup.exe', './dist/app/Aibrowser-' + version + '-setup.exe')
      })
      .catch(err => {
